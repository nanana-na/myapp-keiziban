...
<div class="container">
  <div class="row justify-content-center">
    <div class="col-md-8 mb-3" style="max-width: 700px;">
      <div class="pb-2 bg-white">
        <h1 class="text-center zen p-2"><span>{{ $friend->title }}</span></h1>
      </div>
    </div>
  </div>
  @if ($user_id == $friend->user_id)
  <div class="mb-4 text-center" style="margin-top:10px">
    <a class="btn btn-primary" href="{{ route('friends.edit', $friend->id) }}">
      編集する
    </a>
  ...
  <div class="comment-box w-100">
    <form action="{{ route('comments.store') }}" method="POST">
      ...
      <ul class="comment-ul">
        <li>
          <input type="hidden" name="free_id" value="{{ $friend->id }}">
